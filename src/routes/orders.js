...
import { calculateShipping, normalizeFulfilmentMethod } from "../lib/fulfilment.js";
...
router.post("/",optionalAuth,asyncRoute(async(req,res)=>{
 ...
 const deliveryMethod=normalizeFulfilmentMethod(req.body.deliveryMethod);
 ...
 const client=await pool.connect();try{await client.query("BEGIN");let subtotal=0;const lines=[];
  ...
  const shipping=calculateShipping(subtotal,deliveryMethod);const total=subtotal+shipping;const orderNumber=`HD-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2,6).toUpperCase()}`;
  const shippingAddress=deliveryMethod==="PICKUP"?null:req.body.shippingAddress||req.body.billingAddress||{};
  const order=await client.query(`INSERT INTO orders(order_number,user_id,email,phone,subtotal,shipping_amount,tax_amount,discount_amount,total,currency,billing_address,shipping_address,fulfilment_method,customer_notes,updated_at)
   VALUES($1,$2,$3,$4,$5,$6,0,0,$7,$8,$9,$10,$11,$12,now()) RETURNING *`,[orderNumber,req.user?.id||null,email(req.body.email),text(req.body.phone,"Phone",{max:40}),subtotal,shipping,total,lines[0].currency,req.body.billingAddress||{},shippingAddress,deliveryMethod,text(req.body.customerNotes,"Notes",{max:4000})]);
